...
- **`results_archive/runs/` holds the raw per-run artefacts** — `config.json`, `run.log` and
  every `*/result.json` for **every** run, not just the diagnostics groups (~9,200 files, 38 MB),
  plus `run_metrics_per_seed.csv`. Before this the archive held only aggregates, so no per-seed
  value outside `*_diagnostics` had a backing file — including §1.31's λ\* and §1.10's per-seed
  ...
  (2,418 files, 8.1 GB, SHA-256 each) so an off-cluster copy can be verified.
...
- **Task-vector code lives in `framework/merging/`** — `task_vectors.py` (`task_vector`, `apply_task_vectors`, `merge_task_arithmetic`) and `geometry.py`. There is no `_task_arithmetic_merge` in the pipeline any more. `merge_task_arithmetic` = `apply_task_vectors(base, [task_vector(...)], scale)`; the split exists because a scale sweep builds τ once and re-applies it. **The merge is bitwise reproducible from checkpoints** — recomputing every `merged/checkpoints/best.pt` under `runs/` from its baseline + finetunes gives `torch.equal` on every tensor, **412/412** (`scripts/verify_merge_reproduction.py`, output in `results_archive/audit/verification_log.md`; the previously cited 87/87 predates both the newer run groups and the script — it had none, which is why the number lived in prose). BECAME cells are reported `skipped_became`, not passed: λ\* comes from per-shard Fishers that are not stored, so those merges cannot be rebuilt from checkpoints. Read the *committed* scale (`merge_scale/selected`, falling back to `pipeline_merge_scale`) or selecting runs will look broken. Use that as the regression test for any change here; it is stronger than any unit test you could write.
...
(`--self-test` proves its downgrade rule can fire). The register file itself now holds **39**
rows — `C35`–`C39` are strategy 6 (§1.39), which is not a merging experiment and does not move
