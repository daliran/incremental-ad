...
### 2.27 One α convention for the merge column ✅ — a 20.7% artefact removed from ETTh1 n=3

The merge column in §1.26/§1.5b was built by **two** conventions: most cells select α on the
merged-validation union over 11–25 scales, but `noisefloor_etth` (ETTh1 n=3) merged at a fixed
**α = 1.0** and `exch_incremental` (exchange n=3) at a fixed **0.5**. §1.13 had footnoted the
ETTh1 case and never fixed it. At n=3, α=1.0 is 3× the pre-declared 1/n — the overshoot regime
§1.16 warns about — so that cell measured overshoot, not merging.

Re-run under selection (`selalpha_etth1_n3`, `selalpha_exchange_n3`, 3 seeds each, generated by
`scripts/generate_adfc_grid.py` Part A from the published configs). EXPERIMENTS.md §1.29.

- **ETTh1 n=3: 0.6256 → 0.4964, a decisive 20.7% improvement.** α selected 0.30/0.30/0.35. The
  published cell carried a convention artefact, and §1.13's merge-vs-sequential gap for that cell
  goes from −51.1% to −19.9%.
- **exchange n=3: 0.3310 → 0.3626, a tie.** Its fixed 0.5 was already near the selected 0.55,
  which is why nothing looked wrong there.
- **No winner changed.** Window retraining still wins both rows.

`method_comparison_spec.csv` and `experiment_of_record.csv` now point at the `selalpha_*` runs.
**§1.28 is deliberately left on the old convention** — its 0.3/0.7 rows are fixed-α from the same
reference, so swapping only 0.5 would break that sweep internally.

### 2.28 PSM/SWaT as forecasting benchmarks ✅ — one clean dataset, one failed control

The motivation is §2.25: every forecasting conclusion sits on one arbitrary cut of one series.
PSM and SWaT are the only datasets here that ship a **separate test capture**. New datasets
`PsmForecastDataset` / `SwatForecastDataset`, option C — score only windows where every point,
input span *and* forecast horizon, is labelled normal. 72 runs (2 datasets × {merge, sequential}
× n ∈ {2,3,5} + joint + W ∈ {1,2,3} + 2 training fractions) × 3 seeds, α selected on validation
throughout. EXPERIMENTS.md §1.30.
...
not by being saturated.

**Next.** PSM-forecast deserves the treatment the ETT datasets got: §1.21's window sweep at more
budgets, §1.19/§1.22's prefix analysis, and §1.16's routing headroom — its floor is tight enough
