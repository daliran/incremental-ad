...
#SBATCH --job-name=etth_imputation_standard
...
# --dataset_patch_len must equal --mae_tx_patch_len (enforced by configurator).
# Training is self-supervised random-mask MAE; at evaluation a fixed per-window
# mask is applied and MSE/MAE are measured only at masked positions.
# Output lands in $RUNS_ROOT/<experiment_name>/<run_id>/.
python -m incremental_ad.main \
    --experiment_name mae_tx_etth_imputation \
    --model MaeTx \
    --dataset EtthImputationDataset \
    --task imputation \
    ...
    --mae_tx_n_eval_passes 10 \
    --mae_tx_training_mode random_mask \
    \
    --dataset_window_len 336 \
    --dataset_patch_len 16 \
