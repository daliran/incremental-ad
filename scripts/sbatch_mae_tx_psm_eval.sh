...
# CHECKPOINT: full path to the checkpoint to evaluate.
#   $RUNS_ROOT/<experiment>/<phase>/<run_label>/checkpoints/best.pt
# RUN_TAG: a short label for this eval run.
CHECKPOINT=$RUNS_ROOT/mae_tx_psm/pretrain/<run_label>/checkpoints/best.pt
RUN_TAG=oracle
...
python -m incremental_ad.main \
    ...
    --psm-stride 1 \
    --psm-eval-stride 1 \
    --psm-normalization standard \
    --psm-val-ratio 0.15 \
    --split test \
    --test-eval-seed 0 \
    --test-eval-batch-size 512 \
    --test-eval-threshold-strategy oracle \
    --test-eval-threshold-percentile 72
