...
def _build_psm_mae_tx(
    model_cfg: MaeTxConfig,
    dataset_cfg: PsmConfig,
    train_slice: str,
    partial_ratio: float,
    n_finetune: int,
    base_data_ratio: float,
    ...
    train_dataset, val_dataset, _ = psm.get_loaders(
        dataset_cfg, train_slice, partial_ratio, n_finetune, base_data_ratio
    )
...
def build_for_eval(
    *,
    ...
    if dataset_name == "psm" and model_name == "mae_tx":
        return _build_eval_psm_mae_tx(model_cfg, dataset_cfg, split)
...
def _build_eval_psm_mae_tx(
    model_cfg: MaeTxConfig, dataset_cfg: PsmConfig, split: Split
) -> EvalBuildResult:

    eval_cfg = replace(dataset_cfg, stride=dataset_cfg.eval_stride)

    train_dataset, val_dataset, test_dataset = psm.get_loaders(
        eval_cfg, "full", 1.0, 0, 1.0
    )
    eval_dataset = val_dataset if split == "val" else test_dataset
